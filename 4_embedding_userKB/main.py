...
# ========== ENV VARS ==========
SUPABASE_URL = os.getenv("USR_SUPABASE_URL")
SUPABASE_KEY = os.getenv("USR_SUPABASE_KEY")
DB_CONNECTION = os.getenv("USR_DB_CONNECTION")
SUPABASE_REGION = os.getenv("USR_SUPABASE_REGION", "ap-southeast-1")
SUPABASE_STORAGE_ENDPOINT = os.getenv("USR_SUPABASE_STORAGE_ENDPOINT")
SUPABASE_ACCESS_KEY_ID = os.getenv("USR_SUPABASE_ACCESS_KEY_ID")
SUPABASE_SECRET_ACCESS_KEY = os.getenv("USR_SUPABASE_SECRET_ACCESS_KEY")
...
vec_text = vx.get_or_create_collection(name="user_text", dimension=768)
vec_table = vx.get_or_create_collection(name="user_table", dimension=768)
...
def upload_images_to_bucket(chunks, artifacts_folder: Path, supabase_url,  user_id: str, bucket_name="user.files"):
    updated_chunks = []
    for chunk in chunks:
        ...
        for img_rel_path in image_matches:
            ...
            if full_img_path.exists():
                storage_path = f"{user_id}/{source}_images/{full_img_path.name}"
                try:
                    with open(full_img_path, "rb") as f:
                        s3.upload_fileobj(f, bucket_name, storage_path, ExtraArgs={"ContentType": "image/png"})
...
def store_chunks_in_supabase(chunks, user_id: str, file_name: str):
    ...
    for chunk in chunks:
        ...
        if "content" in chunk:
            ...
            if content.strip():
                ...
                doc_rows.append({"chunk_id": chunk_id, "content": content, "metadata": metadata, "user_id_owner": user_id})
                metadata["user_id_owner"] = user_id
        ...
        if "table" in chunk:
            ...
            tab_rows.append({"chunk_id": chunk_id, "description": description, "metadata": metadata, "user_id_owner": user_id})
            metadata["user_id_owner"] = user_id
            table_records.append((chunk_id, embedding, metadata))

    if doc_rows:
        supabase.table("user_text").insert(doc_rows).execute()
        vec_text.upsert(text_records)
        logger.info(f"✅ Uploaded {len(doc_rows)} text chunks to Supabase")

    if tab_rows:
        supabase.table("user_table").insert(tab_rows).execute()
        vec_table.upsert(table_records)
        logger.info(f"✅ Uploaded {len(tab_rows)} table chunks to Supabase")

def upload_original_pdf(pdf_path: Path, supabase_url, user_id: str, bucket_name="user.files"):
    ...
    storage_path = f"{user_id}/{base_name}/{file_name}"
...
# ========== MAIN RUNNER ==========
def process_json_chunks(json_path: Path, artifacts_folder: Path, original_pdf_path: Path, user_id: str):
    file_name, public_url = upload_original_pdf(original_pdf_path, SUPABASE_URL, user_id)
    ...
    updated_chunks = upload_images_to_bucket(chunks, artifacts_folder, SUPABASE_URL, user_id)
    store_chunks_in_supabase(updated_chunks, user_id, file_name)
...
if __name__ == "__main__":
    ...
    process_json_chunks(JSON_FILE, ARTIFACTS_FOLDER, ORIGINAL_PDF, USER_ID)
